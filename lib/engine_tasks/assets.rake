  ...
  def SKOSXL.for_static_folders
    ...
    if File.directory?(app_public_dir)
      %w(stylesheets javascripts images fonts).each do |specific_dir|
        source_common_dir = File.join(engine_public_dir, specific_dir, "vendor/iqvoc_skosxl")
        if File.exist?(source_common_dir)
          target_common_dir = File.join(app_public_dir, specific_dir, "vendor/iqvoc_skosxl")
