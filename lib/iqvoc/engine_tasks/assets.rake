...
namespace :iqvoc_skosxl do
  namespace :assets do
    ...
    def for_static_folders
      engine_public_dir = Iqvoc::SKOSXL::Engine.find_root_with_flag("public").join('public')
      ...
      if File.directory?(app_public_dir)
        %w(stylesheets javascripts images fonts).each do |specific_dir|
          source_common_dir = File.join(engine_public_dir, specific_dir, "vendor/skosxl")
          if File.exist?(source_common_dir)
            target_common_dir = File.join(app_public_dir, specific_dir, "vendor/skosxl")
